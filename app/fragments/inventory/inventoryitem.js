...
var InventoryInfo = React.createClass({
    ...
    getDialogItem: function (index) {
        ...
        return (
            <div className="instockdetail_dialog_item">
    ...
    render: function () {
        ...
        return (
            <tr style = {{ color: "#979797" }} >
                <td><span>{this.props.itemObj.warehouse_name == '' ? '仓库名缺失' : this.props.itemObj.warehouse_name}</span></td>
                <td><span>{avail}</span></td>
                <td><span>{occupancy}</span></td>
                <td onClick={this.showRecord}><span style = {{ color: "#039FFF" }}> {actual} </span> </td >
            </tr>
        );
    }
});
var InventoryItem = React.createClass({
    ...
    itemOnClick: function () {
        // console.log(this.props.itemObj,"dddddd");
        var params={
           data: this.props.itemObj
        };
        gVar.pushPage({ pathname: "instockdetail", state: params}, true);
    ...
    render: function () {
        ...
        return (<div style = {{
            ...
            <div style = {{ padding: "0px 8px", color: "#4A4A4A" }} > 商品编码: {this.props.itemObj.upc} </div>
            <div style = {{ padding: "0px 8px", color: "#979797" }} >{this.props.itemObj.name} </div>
            <table className = "table inventory_item_info" style = {{ width: "100%", marginBottom: "0px" }}>
                <thead>
                    <tr >
